...
function AccountSettings({ user, onClose, toast }) {
  ...
  return (
    <Modal title="Paramètres du compte" onClose={onClose}>
      <div style={{ display:'flex', gap:4, background:'rgba(255,126,95,.12)', padding:4, borderRadius:8, marginBottom:20 }}>
        {tabs.map(({ key, label }) => (
          <button
            key={key}
            type="button"
      ...
      {tab==='profil' && (
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:14, padding:16, background:'rgba(255,126,95,.12)', borderRadius:12, marginBottom:20 }}>
        ...
        {config.map((s, i) => (
          <button
            ...
            style={{
              ...
              cursor:'pointer',
              fontFamily:'inherit',
              background:i===activeIdx ? `linear-gradient(135deg,${DS.primary},${DS.primary2})` : 'rgba(255,126,95,.14)',
