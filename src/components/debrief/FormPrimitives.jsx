...
function RadioGroup({ label, options, value, onChange }) {
  return (
    <div style={{ marginBottom: 18 }}>
      {label && <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--txt)', marginBottom: 9 }}>{label}</p>}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {options.map(opt => (
          <label
            key={opt.value}
            onClick={e => { e.preventDefault(); onChange(value === opt.value ? '' : opt.value); }}
            style={{
              display: 'flex', alignItems: 'flex-start', gap: 10,
              padding: '11px 14px', borderRadius: 10,
              border: `1px solid ${value === opt.value ? 'rgba(255,126,95,.18)' : 'var(--border)'}`,
              background: value === opt.value ? 'var(--surface-accent)' : 'var(--glass-bg)',
              cursor: 'pointer', fontSize: 13,
              color: value === opt.value ? 'var(--txt)' : 'var(--txt2)',
              transition: 'all .15s',
              boxShadow: value === opt.value ? '0 4px 12px rgba(255,126,95,.08)' : 'none',
              backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)',
            }}
          >
            <input type="radio" style={{ marginTop: 3, accentColor: P, flexShrink: 0 }} checked={value === opt.value} readOnly />
            <span>{opt.label}</span>
          </label>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {options.map(opt => (
          <label key={opt.value} style={{
            display: 'flex', alignItems: 'flex-start', gap: 10,
            padding: '11px 14px', borderRadius: 10,
            border: `1px solid ${value.includes(opt.value) ? 'rgba(255,126,95,.18)' : 'var(--border)'}`,
            background: value.includes(opt.value) ? 'var(--surface-accent)' : 'var(--glass-bg)',
            cursor: 'pointer', fontSize: 13,
            color: value.includes(opt.value) ? 'var(--txt)' : 'var(--txt2)',
            transition: 'all .15s',
            boxShadow: value.includes(opt.value) ? '0 4px 12px rgba(255,126,95,.08)' : 'none',
            backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)',
          }}>
            <input type="checkbox" style={{ marginTop: 3, accentColor: P, flexShrink: 0 }} checked={value.includes(opt.value)} onChange={() => toggle(opt.value)} />
            <span>{opt.label}</span>
          </label>
        ))}
      </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : 'repeat(3,1fr)', gap: 10, paddingTop: 16, marginTop: 8, borderTop: '1px solid var(--border)' }}>
      {[
        ...
        <div key={key}>
          ...
          <textarea rows={mob ? 2 : 3} placeholder={placeholder} value={notes[key] || ''} onChange={e => onChange({ ...notes, [key]: e.target.value })} style={{
            width: '100%', borderRadius: 8, border: '1px solid var(--border)',
            background: 'var(--input-on-card)', color: 'var(--txt)', padding: '7px 10px',
            fontSize: 12, resize: 'none', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box',
          }} />
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ ...cardSm({ border: '1px solid var(--border)' }), overflow: 'hidden', marginBottom: 12 }}>
      <button type="button" onClick={() => setOpen(v => !v)} style={{
        ...
        background: open ? 'var(--gradient-primary)' : 'var(--glass-bg)',
        width: '100%', border: 'none', cursor: 'pointer', fontFamily: 'inherit', transition: 'all .2s',
      }}>
        <span style={{
          width: 26, height: 26, borderRadius: '50%',
          background: open ? 'rgba(255,255,255,.22)' : 'var(--gradient-primary)',
          color: 'white', fontSize: 11, fontWeight: 700,
          display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
        }}>{number}</span>
        <span style={{ fontWeight: 700, fontSize: 14, color: open ? 'white' : 'var(--txt)', flex: 1, textAlign: 'left' }}>{title}</span>
        <span style={{ fontSize: 13, color: open ? 'rgba(255,255,255,.8)' : 'var(--txt3)', transition: 'transform .2s', display: 'inline-block', transform: open ? 'rotate(180deg)' : 'none' }}>{'▾'}</span>
      </button>
...
function S1({ data = {}, onChange, notes, onNotes }) {
  ...
  return (
    <CatCard number="1" title="Phase de découverte">
      ...
      <RadioGroup label="Douleur profonde / identitaire atteinte ?" options={[{ value: 'oui', label: 'Oui — verbalisé fort' }, { value: 'partiel', label: 'Partiellement' }, { value: 'non', label: 'Non' }]} value={data.douleur_profonde} onChange={v => set('douleur_profonde', v)} />
      ...
      <RadioGroup label="Urgence naturelle identifiée ?" options={[{ value: 'oui', label: 'Oui' }, { value: 'artificielle', label: 'Artificielle' }, { value: 'aucune', label: 'Aucune' }]} value={data.urgence} onChange={v => set('urgence', v)} />
...
function S2({ data = {}, onChange, notes, onNotes }) {
  ...
  return (
    <CatCard number="2" title="Reformulation">
      <RadioGroup label="Reformulation faite ?" options={[{ value: 'oui', label: 'Complète et précise' }, { value: 'partiel', label: 'Partielle' }, { value: 'non', label: 'Non' }]} value={data.reformulation} onChange={v => set('reformulation', v)} />
      <RadioGroup label="Le prospect s'est reconnu ?" options={[{ value: 'oui', label: "Oui — \"c'est exactement ça\"" }, { value: 'moyen', label: 'Moyen' }, { value: 'non', label: 'Non' }]} value={data.prospect_reconnu} onChange={v => set('prospect_reconnu', v)} />
...
function S3({ data = {}, onChange, notes, onNotes }) {
  ...
  return (
    <CatCard number="3" title="Projection">
      ...
      <RadioGroup label="Qualité de la réponse" options={[{ value: 'forte', label: 'Forte — émotionnelle, identitaire' }, { value: 'moyenne', label: 'Moyenne' }, { value: 'faible', label: 'Faible' }]} value={data.qualite_reponse} onChange={v => set('qualite_reponse', v)} />
      <RadioGroup label="Deadline utilisée comme levier ?" options={[{ value: 'oui', label: 'Oui' }, { value: 'non_exploitee', label: 'Non exploitée' }, { value: 'pas_de_deadline', label: 'Pas de deadline' }]} value={data.deadline_levier} onChange={v => set('deadline_levier', v)} />
      <SectionNotes notes={notes} onChange={onNotes} />
    </CatCard>
  );
}
...
function S4({ data = {}, onChange, notes, onNotes }) {
  ...
  return (
    <CatCard number="4" title="Présentation de l'offre">
      <RadioGroup label="Présentation collée aux douleurs ?" options={[{ value: 'oui', label: 'Oui' }, { value: 'partiel', label: 'Partiellement' }, { value: 'non', label: 'Non — générique' }]} value={data.colle_douleurs} onChange={v => set('colle_douleurs', v)} />
      <RadioGroup label="Exemples bien choisis ?" options={[{ value: 'oui', label: "Oui — le prospect s'est reconnu" }, { value: 'moyen', label: 'Moyen' }, { value: 'non', label: 'Non' }]} value={data.exemples_transformation} onChange={v => set('exemples_transformation', v)} />
      <RadioGroup label="Durée / Offre justifiée ?" options={[{ value: 'oui', label: 'Oui' }, { value: 'partiel', label: 'Partiellement' }, { value: 'non', label: 'Non' }]} value={data.duree_justifiee} onChange={v => set('duree_justifiee', v)} />
      <SectionNotes notes={notes} onChange={onNotes} />
    </CatCard>
  );
}
...
function S5({ data = {}, onChange, notes, onNotes }) {
  ...
  return (
    <CatCard number="5" title="Closing & Objections">
      <RadioGroup label="Annonce du prix" options={[{ value: 'directe', label: 'Directe et assumée' }, { value: 'hesitante', label: 'Hésitante' }, { value: 'trop_rapide', label: 'Trop rapide' }]} value={data.annonce_prix} onChange={v => set('annonce_prix', v)} />
      ...
      <RadioGroup label="Résultat du closing" options={[{ value: 'close', label: 'Closé en direct' }, { value: 'retrograde', label: 'Rétrogradé' }, { value: 'relance', label: 'Relance planifiée' }, { value: 'porte_ouverte', label: 'Porte ouverte' }, { value: 'perdu', label: 'Perdu' }]} value={data.resultat_closing} onChange={v => set('resultat_closing', v)} />
      <SectionNotes notes={notes} onChange={onNotes} />
    </CatCard>
  );
}
