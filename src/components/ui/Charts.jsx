...
export function Radar({ scores, compareScores = null, color = '#FF7E5F', compareColor = '#7C3AED', size = 260 }) {
  ...
  const fill = color === '#059669' ? 'rgba(5,150,105,.14)' : 'rgba(255,126,95,.14)';
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ overflow: 'visible', maxWidth: '100%' }}>
      {[1, 2, 3, 4, 5].map(l => {
        ...
        return <polygon key={l} points={ps} fill={l === 5 ? 'rgba(200,160,140,.06)' : 'none'} stroke="rgba(200,160,140,.12)" strokeWidth="1" />;
      })}
      {SECTIONS.map((_, i) => { const a = angle(i); return <line key={i} x1={cx} y1={cy} x2={cx + R * Math.cos(a)} y2={cy + R * Math.sin(a)} stroke="rgba(200,160,140,.12)" strokeWidth="1" />; })}

      {hasCompare && (
        <polygon points={comparePts.map(p => p.join(',')).join(' ')} fill={`${compareColor}18`} stroke={compareColor} strokeWidth="2" strokeDasharray="4 4" />
      )}

      {!allZero && (
        <>
          <polygon points={pts.map(p => p.join(',')).join(' ')} fill={fill} stroke={color} strokeWidth="2.4" />
      ...
      {SECTIONS.map((s, i) => {
        ...
        return (
          <text key={i}
            ...
            fontSize="9.5" fill="var(--txt2, #B09080)" fontWeight="600"
          >
            {shortLabel}
          </text>
        );
      ...
      <circle cx={cx} cy={cy} r={3} fill="rgba(200,160,140,.3)" />
    </svg>
  );
}
        ...
        return (
          <div key={key}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--txt)' }}>{LABELS[key]}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {globalScores && <span style={{ fontSize: 11, color: 'var(--txt3)' }}>{glob}/5 global</span>}
                <span style={{
                  fontSize: 12, fontWeight: 700, padding: '3px 8px', borderRadius: 999,
                  border: '1px solid var(--border)', background: 'var(--chip-bg)', color: col(val),
                }}>{val}/5</span>
                {diff !== null && diff !== 0 && <span style={{ fontSize: 11, fontWeight: 600, color: diff > 0 ? '#059669' : '#EF4444' }}>{diff > 0 ? '+' : ''}{diff}</span>}
              </div>
            </div>
            <div style={{ position: 'relative', height: 8, background: 'rgba(200,160,140,.06)', borderRadius: 999, overflow: 'visible' }}>
              <div style={{ height: '100%', width: `${(val / 5) * 100}%`, background: col(val), borderRadius: 4, transition: 'width .7s ease' }} />
              {globalScores && glob > 0 && <div style={{ position: 'absolute', top: -1, left: `${(glob / 5) * 100}%`, width: 2, height: 10, background: 'var(--txt3)', borderRadius: 2 }} title={`Global: ${glob}/5`} />}
            </div>
          </div>
        );
      })}
