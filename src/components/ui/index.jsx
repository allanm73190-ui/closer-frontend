...
export function Toasts({ list }) {
  ...
  const bg = { success:'linear-gradient(135deg,#e87d6a,#d4604e)', error:'linear-gradient(135deg,#ef4444,#dc2626)', info:'linear-gradient(135deg,#3a7a9a,#0e7490)' };
  ...
  return (
    <div style={{ position:'fixed', bottom:mob?18:26, right:mob?10:24, left:mob?10:'auto', zIndex:9999, display:'flex', flexDirection:'column', gap:8 }}>
      {list.map(t => (
        <div key={t.id} style={{ padding:'12px 16px', borderRadius:12, fontSize:13, fontWeight:600, background:bg[t.type]||bg.success, color:'white', display:'flex', alignItems:'center', gap:8, animation:'toastIn .25s ease', boxShadow:'0 14px 30px rgba(16,18,28,.22)', border:'1px solid rgba(255,255,255,.24)' }}>
...
export function Burst({ points, levelUp, newLevel, onDone }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9998, pointerEvents:'none', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:`linear-gradient(135deg,${P},${P2})`, borderRadius:22, padding:'26px 38px', color:'white', textAlign:'center', animation:'burstIn .4s cubic-bezier(.34,1.56,.64,1)', boxShadow:'0 18px 42px rgba(232,125,106,.42)', border:'1px solid rgba(255,255,255,.28)' }}>
...
export function Input({ placeholder, value, onChange, type='text', required, autoFocus, style={} }) {
  ...
  return (
    <input type={type} placeholder={placeholder} value={value} onChange={onChange}
      ...
      style={{ ...inp(), borderColor: focus?P:'var(--border)',
        transform: focus ? 'translateY(-1px)' : 'none',
        boxShadow: focus ? SH_IN+', 0 0 0 3px rgba(232,125,106,.12), 0 8px 18px rgba(232,125,106,.15)' : SH_IN, ...style }}
...
// ─── BUTTON ──────────────────────────────────────────────────────────────────
const BTN = {
  primary:   { background:`linear-gradient(135deg,${P},${P2})`, color:'white', border:'1px solid rgba(255,255,255,.22)', boxShadow:SH_BTN },
...
export function Btn({ children, onClick, type='button', variant='primary', disabled, style={} }) {
  return (
    <button type={type} onClick={onClick} disabled={disabled} style={{
      ...
      gap:8, padding:'10px 20px', minHeight:38, borderRadius:R_FULL, fontSize:13, fontWeight:700, letterSpacing:'.01em',
...
export function Card({ children, style={} }) {
  return <div style={{ ...card({ backdropFilter:'blur(8px)' }), ...style }}>{children}</div>;
}
...
export function Modal({ title, onClose, children }) {
  const mob = useIsMobile();
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9000, background:'rgba(20,28,40,.5)', display:'flex', alignItems:'center', justifyContent:'center', padding:mob?8:20, backdropFilter:'blur(6px)' }}
      onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={{ background:'var(--card,#ffffff)', borderRadius:mob?16:20, padding:mob?16:22, width:mob?'calc(100vw - 16px)':'min(680px, calc(100vw - 40px))', maxHeight:'92vh', overflowY:'auto', boxShadow:'var(--sh-card)', border:'1px solid var(--border)' }}>
  ...
  return (
    <div style={{ position:'relative', display:'inline-flex', alignItems:'center', justifyContent:'center' }}>
      <svg width={vb} height={vb} viewBox={`0 0 ${vb} ${vb}`}>
        <circle cx={c} cy={c} r={r} fill="none" stroke="rgba(148,163,184,.2)" strokeWidth={st}/>
